...
> Ultra-fast, lightweight, and keyboard-driven BitTorrent search & download TUI for Linux. Built entirely in Rust.
...
[![Binary Size](https://img.shields.io/badge/binary--size-4.0_MB-blueviolet.svg?style=flat-square)](#)
...
- **Compact 4.0 MB Binary Size:** Compiled utilizing global Link-Time Optimization (LTO), panic-aborts, and Native-TLS feature pruning.
...
| :--- | :--- | :--- |
| `src/main.rs` | Entry point of the program. Loads application configuration and starts main loop. | `main` |
| `src/lib.rs` | Module declarations making components reusable. | Module list |
| `src/app.rs` | Coordinates main execution flow, starting Session and passing indexers/history to TUI. | `App` |
| `src/tui.rs` | Manages Terminal User Interface, event loops, visual rendering, key handling, and downloads. | `SearchTui`, `run_search_tui` |
| `src/indexer.rs` | Integrates with public torrent indexers (PirateBay API, Nyaa RSS feed scraping). | `search_piratebay`, `search_nyaa` |
| `src/storage.rs` | Serializes, deserializes, and prunes local download history records. | `DownloadHistoryEntry` |
| `src/types.rs` | Data structures representing configuration, search results, and serialized download states. | `Torrent` |
| `src/util.rs` | Size formatting, URL encoding, magnet link assembly, and fallback JSON deserialization. | `format_size`, `build_magnet_link` |
...
cp target/release/torrentty ~/.local/bin/torrenty
```
