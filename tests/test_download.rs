...
#[tokio::test]
#[ignore = "requires live external BitTorrent swarm"]
async fn test_nyaa_search_and_metadata_fetch() {
    ...
    let results = search_nyaa(&client, "SubsPlease Slime 20 1080p", 10)
        .await
        .expect("search nyaa");
    ...
    assert!(
        top.torrent_url.is_some(),
        "expected torrent_url to be extracted"
    );
    ...
    assert!(
        bytes.len() > 1000,
        "torrent file should be valid bencoded data"
    );
    println!(
        "Successfully verified .torrent fetch ({} bytes)!",
        bytes.len()
    );
    ...
    let resp = session
        .add_torrent(AddTorrent::TorrentFileBytes(bytes), Some(add_opts))
        .await
        .expect("add torrent");
    ...
    assert!(
        ...
        "torrent addition should not deadlock"
    );
    ...
    assert!(
        stats.total_bytes > 0,
        "metadata should be populated immediately from .torrent bytes"
    );
    ...
    for i in 0..60 {
        ...
        let peer_count = live.map_or(0, |l| l.snapshot.peer_stats.live);
        let fetched_bytes = live.map_or(0, |l| l.snapshot.fetched_bytes);
        println!(
        ...
        if stats.progress_bytes > 0 || fetched_bytes > 0 {
            println!(
                "SUCCESS: Downloaded bytes of Uma Musume Cinderella! fetched={}, progress={}",
                fetched_bytes, stats.progress_bytes
            );
            break;
        }
    }
